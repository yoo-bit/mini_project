...
while 1:
	...
	#	p.setJointMotorControl2(bodyUniqueId=boxId,jointIndex=0, controlMode=p.VELOCITY_CONTROL, targetVelocity = 1, force = 500)
		...
		if (toeLinkHeight > 0) and (toeLinkHeight < 0.2):
			...
			##GoodValue set
			controlBodyAttitude(physicsClass=p,phi=pitchAnglephi,phiDesired=0,phiDerivative=pitchAnglePhiDerivative,kp=2503,kv=2400)
		...
		# 	# print("FLIGHT")
			...
			desiredAngle = calculateDesiredLegAndBodyAngle(phi=pitchAnglephi,forwardSpeed=forwardVelocity,desiredForwardSpeed=forwardVelocity+0.1,stancePhaseDuration=averageStanceDuration,r=legLength,feedBackGain=0.5)
